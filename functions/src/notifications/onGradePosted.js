/**
 * Grade Alert Notification Trigger
 * 
  ...
  .onWrite(async (change, context) => {
    ...
    try {
      ...
      const notificationPromises = parentsSnap.docs.map(async (parentDoc) => {
        ...
        console.log(`Sending grade email to ${parent.email} for ${studentName}, Q${quarter}`);
      ...
      console.log(`Grade notification complete for student ${studentId}, Q${quarter}: ${sentCount} sent, ${failedCount} failed`);
...
/**
 * Manual retry function for failed grade notifications
 * Callable from admin dashboard
 */
exports.retryGradeNotification = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // Rebuild grade summary
    const gradeSummary = await buildGradeSummary(
      notif.studentId, 
      notif.metadata.quarter
    );
    ...
    const emailResult = await sendEmail({
      ...
      subject: `Quarter ${notif.metadata.quarter} Grades Posted for ${studentName}`,
      html: emailContent.html,
      text: emailContent.text
    });
...
exports.sendGradeNotificationManual = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // This will run through the normal onGradePosted trigger
    const gradesSnap = await admin.firestore()
      .collection('grades')
      .where('studentId', '==', studentId)
      .where('quarter', '==', quarter)
