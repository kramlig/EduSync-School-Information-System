/**
 * Division SF6 PDF Generator - Consolidated Enrollment Summary
 * 
 * Generates DepEd-compliant Division-level SF6 report summarizing
 * enrollment data across all schools in a division
 */

import jsPDF from 'jspdf';
import type { DivisionEnrollmentAggregate } from '../../services/divisionReportService';
import depedSealUrl from '../../assets/deped-logo.png';
import depedLogoUrl from '../../assets/deped-seal.png';

// Types
export interface DivisionSF6PDFOptions {
...
function renderHeader(
  ...
  doc.text('DIVISION CONSOLIDATED SF6', pageWidth / 2, 22, { align: 'center' });
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(FONT_SIZES.BODY);
  doc.text('Summarized Report on Enrollment', pageWidth / 2, 27, { align: 'center' });
  ...
  doc.text(`Date Prepared: ${options.date_prepared || new Date().toLocaleDateString()}`, pageWidth - MARGINS.RIGHT - 50, y);
...
function renderSummaryCards(
  doc: jsPDF,
  y: number,
  data: DivisionEnrollmentAggregate
): number {
  const cardWidth = 45;
  const cardHeight = 20;
  ...
  const cards = [
    { label: 'Total Schools', value: data.total_schools.toLocaleString(), color: COLORS.PRIMARY },
    { label: 'Total Students', value: data.total_students.toLocaleString(), color: COLORS.PRIMARY },
    { label: 'Male', value: data.total_male.toLocaleString(), color: COLORS.MALE },
    { label: 'Female', value: data.total_female.toLocaleString(), color: COLORS.FEMALE },
  ];
...
/**
 * Render enrollment by grade level table
 */
function renderByGradeTable(
  ...
  const colWidths = [50, 40, 40, 40, 20]; // Grade, Male, Female, Total, %
  const rowHeight = 6;

  // Headers
  const headers = ['Grade Level', 'Male', 'Female', 'Total', '%'];
  ...
  grades.forEach((grade) => {
    ...
    const percentage = data.total_students > 0
      ? ((gradeData.total / data.total_students) * 100).toFixed(1)
      : '0.0';

    const values = [
      gradeLabel,
      gradeData.male.toLocaleString(),
      gradeData.female.toLocaleString(),
  ...
  // Draw table border
  doc.setDrawColor(...COLORS.BORDER);
  doc.rect(MARGINS.LEFT, y - (grades.length + 2) * rowHeight, tableWidth, (grades.length + 2) * rowHeight);

  return y + 5;
}

/**
 * Render enrollment by district table
 */
function renderByDistrictTable(
  ...
  const headers = ['District', 'Schools', 'Male', 'Female', 'Total'];
  ...
  districts.forEach(([district, districtData], index) => {
    ...
    const values = [
      ...
      districtData.male.toLocaleString(),
      districtData.female.toLocaleString(),
...
function renderSchoolSummary(
  ...
  const tableWidth = PAGE_CONFIG.WIDTH - MARGINS.LEFT - MARGINS.RIGHT;
  const colWidths = [80, 35, 35, 40];
  const rowHeight = 5;

  const headers = ['School Name', 'Male', 'Female', 'Total'];
  ...
  schools.forEach((school, index) => {
    ...
    if (schoolName.length > 45) {
      schoolName = schoolName.substring(0, 42) + '...';
    }
 ...
 */
function renderSignatures(doc: jsPDF, y: number, options: DivisionSF6PDFOptions): void {
  const pageWidth = PAGE_CONFIG.WIDTH;
  const colWidth = 80;
 ...
 */
export async function generateDivisionSF6PDF(
  data: DivisionEnrollmentAggregate,
  options: DivisionSF6PDFOptions
): Promise<jsPDF> {
  const doc = new jsPDF({
  ...
  y = renderSectionTitle(doc, 'ENROLLMENT BY GRADE LEVEL', y);
  y = renderByGradeTable(doc, y, data);

  y = renderSectionTitle(doc, 'ENROLLMENT BY DISTRICT', y);
 ...
 */
export async function downloadDivisionSF6PDF(
  data: DivisionEnrollmentAggregate,
  options: DivisionSF6PDFOptions
): Promise<void> {
  const doc = await generateDivisionSF6PDF(data, options);
  const filename = `Division_SF6_${options.division_name.replace(/\s+/g, '_')}_${options.school_year}.pdf`;
  doc.save(filename);
}
