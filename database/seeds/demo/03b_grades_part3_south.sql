-- ============================================================================
-- SEED DEMO GRADES - PART 3: MATI SOUTH DISTRICT
-- Run after parts 1 and 2
-- ============================================================================

-- Clear existing grades for South district first
DELETE FROM grades WHERE school_id IN (
  SELECT id FROM schools WHERE division = 'Division of City of Mati' AND district ILIKE '%South%'
);
...
WHERE s.division = 'Division of City of Mati'
  AND s.district ILIKE '%South%'
  AND st.grade_level = ANY(la.grade_levels);

DROP TABLE IF EXISTS school_performance;

SELECT 'Mati South grades created' as status, COUNT(*) as count 
FROM grades g JOIN schools s ON g.school_id = s.id
WHERE s.district ILIKE '%South%';

-- Final verification
SELECT 'TOTAL grades' as status, COUNT(*) as count FROM grades g
